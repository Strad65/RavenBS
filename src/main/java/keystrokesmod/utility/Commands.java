...
public class Commands {
   ...
   private static final List<Integer> cs = Arrays.asList(
   ...
   public static void rCMD(String c) {
      if (!c.isEmpty()) {
         ...
         if (cm.startsWith("setkey".toLowerCase())) {
            ...
            String n = c.replaceFirst("fakechat", "").substring(1);
            ...
            keystrokesmod.Raven.getScheduledExecutor().execute(() -> {
               ...
               if (s != null) {
                  if (s[0] == -1) {
                     ...
                     double wlr = s[1] != 0 ? Utils.round((double)s[0] / s[1], 2) : s[0];
                     print("&e" + n + " stats:", 1);
                     print("Wins: " + s[0], 0);
                     print("Losses: " + s[1], 0);
                     print("WLR: " + wlr, 0);
            ...
            if (!cm.startsWith("enemy") && !cm.startsWith("e")) {
               if (cm.startsWith("Debug".toLowerCase())) {
                  keystrokesmod.Raven.debug = !keystrokesmod.Raven.debug;
                  print("Debug " + (keystrokesmod.Raven.debug ? "enabled" : "disabled") + ".", 1);
               } else if (!cm.startsWith("profiles") && !cm.startsWith("p")) {
                  if (cm.startsWith("help") || cm.startsWith("?")) {
                  ...
                  } else if (cm.startsWith("shoutout")) {
                     ...
                     print("&cInvalid command. (" + (cm.length() > 5 ? cm.substring(0, 5) + "..." : cm) + ")", 1);
                  }
               } else if (!hasArgs) {
                  print("&aAvailable profiles:", 1);
                  if (keystrokesmod.Raven.profileManager.profiles.isEmpty()) {
                     print("None", 0);
                     return;
                  }

                  for (int i = 0; i < keystrokesmod.Raven.profileManager.profiles.size(); i++) {
                     print(i + 1 + ". " + keystrokesmod.Raven.profileManager.profiles.get(i).getName(), 0);
                  }
               } else if (args.length > 1) {
                  if (!args[1].equals("save") && !args[1].equals("s")) {
                     if (!args[1].equals("load") && !args[1].equals("l")) {
                        if (args[1].equals("remove") || args[1].equals("r")) {
                           if (args.length != 3) {
                              print("&cInvalid syntax.", 1);
                              return;
                           }
                           ...
                           for (Profile profile : keystrokesmod.Raven.profileManager.profiles) {
                              if (profile.getName().equals(name)) {
                                 keystrokesmod.Raven.profileManager.deleteProfile(profile.getName());
                        ...
                        for (Profile profile : keystrokesmod.Raven.profileManager.profiles) {
                           if (profile.getName().equals(name)) {
                              keystrokesmod.Raven.profileManager.loadProfile(profile.getName());
   ...
   public static void rc(FontRenderer fr, int h, int w, double s) {
      ...
      GL11.glEnable(3089);
      double mw = w * s;
      GL11.glScissor(0, (int)(mc.displayHeight - (sY + sH) * s), (int)(mw - (mw < 2.0 ? 0 : 2)), (int)(sH * s - 2.0));
