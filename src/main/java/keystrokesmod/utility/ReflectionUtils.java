...
public class ReflectionUtils {
   ...
   private static List<Class> containerClasses = Arrays.asList(
      GuiFurnace.class,
   ...
   private static void addToMap(Class clazz, Field field) {
      if (field != null && field.getType() == IInventory.class) {
         field = ReflectionHelper.findField(clazz, new String[]{field.getName()});
         if (field != null) {
   ...
   public static boolean setItemInUse(boolean blocking) {
      if (blocking || !ModuleManager.killAura.blockingClient && !ModuleManager.noSlow.blockingClient) {
         ((IAccessorEntityPlayer)Minecraft.getMinecraft().thePlayer).setItemInUseCount(blocking ? 999 : 0);
         return blocking;
      } else {
         return blocking;
      }
