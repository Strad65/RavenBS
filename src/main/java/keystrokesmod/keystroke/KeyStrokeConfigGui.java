...
public class KeyStrokeConfigGui extends GuiScreen {
   ...
   public void initGui() {
      this.buttonList
         .add(
            this.modeBtn = new GuiButton(
               0, this.width / 2 - 70, this.height / 2 - 28, 140, 20, "Mod: " + (KeyStroke.e ? "Enabled" : "Disabled")
            )
         );
      this.buttonList
         .add(
            this.textColorBtn = new GuiButton(
               1, this.width / 2 - 70, this.height / 2 - 6, 140, 20, "Text color: " + colors[KeyStroke.currentColorNumber]
            )
         );
      this.buttonList
         .add(
            this.showMouseBtn = new GuiButton(
               2, this.width / 2 - 70, this.height / 2 + 16, 140, 20, "Show mouse buttons: " + (KeyStroke.d ? "On" : "Off")
            )
         );
      this.buttonList
         .add(
            this.outlineBtn = new GuiButton(3, this.width / 2 - 70, this.height / 2 + 38, 140, 20, "Outline: " + (KeyStroke.f ? "On" : "Off"))
         );
   }

   public void drawScreen(int mouseX, int mouseY, float partialTicks) {
      keystrokesmod.Raven.getKeyStrokeRenderer().renderKeystrokes();
