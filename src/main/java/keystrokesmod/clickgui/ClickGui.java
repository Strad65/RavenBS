...
public class ClickGui extends GuiScreen {
   ...
   public void initMain() {
      ...
      this.sf = keystrokesmod.Raven.getScheduledExecutor().schedule(() -> (this.logoSmoothLength = new Timer(650.0F)).start(), 650L, TimeUnit.MILLISECONDS);
   }
   ...
   public void drawScreen(int x, int y, float p) {
      if (Gui.backgroundBlur.getInput() != 0.0) {
         BlurUtils.prepareBlur();
         RoundedUtils.drawRound(0.0F, 0.0F, this.width, this.height, 0.0F, true, Color.black);
         float inputToRange = (float)(3.0 * ((Gui.backgroundBlur.getInput() + 35.0) / 100.0));
         BlurUtils.blurEnd(2, this.blurSmooth.getValueFloat(0.0F, inputToRange, 1));
      }
   ...
   public void setWorldAndResolution(Minecraft p_setWorldAndResolution_1_, int p_setWorldAndResolution_2_, int p_setWorldAndResolution_3_) {
      ...
      this.originalScale = this.mc.gameSettings.guiScale;
      ...
      if (!MinecraftForge.EVENT_BUS.post(new Pre(this, this.buttonList))) {
         this.buttonList.clear();
         this.initGui();
      }

      MinecraftForge.EVENT_BUS.post(new Post(this, this.buttonList));
   }
   ...
   public void onGuiClosed() {
      ...
      this.mc.gameSettings.guiScale = this.originalScale;
   }
