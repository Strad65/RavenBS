...
public abstract class MixinRendererLivingEntity<T extends EntityLivingBase> extends Render<T> {
   ...
   @Redirect(
      ...
      at = @At(
         value = "INVOKE",
         target = "Lnet/minecraft/client/renderer/entity/RendererLivingEntity;setScoreTeamColor(Lnet/minecraft/entity/EntityLivingBase;)Z"
      )
   )
   private boolean setOutlineColor(RendererLivingEntity instance, T entityLivingBaseIn) {
      int i = 16777215;
      boolean drawOutline = this.shouldRender()
         && (
            entityLivingBaseIn != Minecraft.getMinecraft().thePlayer && !AntiBot.isBot(entityLivingBaseIn)
               || entityLivingBaseIn == Minecraft.getMinecraft().thePlayer && ModuleManager.playerESP.renderSelf.isToggled()
         );
      if (drawOutline && !ModuleManager.playerESP.teamColor.isToggled()) {
         if (ModuleManager.playerESP.rainbow.isToggled()) {
            ...
            i = new Color(
                  (int)ModuleManager.playerESP.red.getInput(), (int)ModuleManager.playerESP.green.getInput(), (int)ModuleManager.playerESP.blue.getInput()
               )
               .getRGB();
         }
   ...
   @ModifyVariable(method = "renderModel", at = @At("STORE"), ordinal = 0)
   private boolean modifyInvisibleFlag(boolean flag) {
      return flag || this.renderOutlines && this.shouldRender() && ModuleManager.playerESP.showInvis.isToggled();
   }
}
