...
public class SumoFences extends Module {
   ...
   private static final List<BlockPos> f_p = Arrays.asList(
      new BlockPos(9, 65, -2),
      new BlockPos(9, 65, -1),
      new BlockPos(9, 65, 0),
      new BlockPos(9, 65, 1),
   ...
   public SumoFences() {
      ...
      this.registerSetting(fenceHeight = new SliderSetting("Fence height", 4.0, 1.0, 6.0, 1.0));
      this.registerSetting(this.blockType = new SliderSetting("Block type", 0, this.mode));
   }
            ...
            if (SumoFences.this.isSumo()) {
               for (BlockPos p : SumoFences.f_p) {
                  for (int i = 0; i < SumoFences.fenceHeight.getInput(); i++) {
                     BlockPos p2 = new BlockPos(p.getX(), p.getY() + i, p.getZ());
                     if (SumoFences.mc.theWorld.getBlockState(p2).getBlock() == Blocks.air) {
                        SumoFences.mc.theWorld.setBlockState(p2, SumoFences.this.f);
                     }
                  }
               }
            }
   ...
   public void guiUpdate() {
      switch ((int)this.blockType.getInput()) {
         case 0:
            this.f = Blocks.oak_fence.getDefaultState();
            break;
         case 1:
