...
public class Reach extends Module {
   ...
   public void e(MouseEvent ev) {
      if (ev.button >= 0
         && ev.buttonstate
         && Utils.nullCheck()
         && (!ModuleManager.autoClicker.isEnabled() || !AutoClicker.leftClick.isToggled() || !Mouse.isButtonDown(0))) {
         call();
      }
   }
   ...
   public static boolean call() {
      ...
      if (movingOnly.isToggled() && mc.thePlayer.moveForward == 0.0 && mc.thePlayer.moveStrafing == 0.0) {
         return false;
      }
   ...
   private static Object[] getEntity(double reach, double expand) {
      if (!ModuleManager.reach.isEnabled()) {
         reach = mc.playerController.extendedReach() ? 6.0 : 3.0;
      }
   ...
   public static Object[] getEntity(double reach, double expand, float[] rotations) {
      ...
      Vec3 hitVec = null;
      List zz8 = mc.theWorld
         .getEntitiesWithinAABBExcludingEntity(
            zz2, zz2.getEntityBoundingBox().addCoord(zz4.xCoord * reach, zz4.yCoord * reach, zz4.zCoord * reach).expand(1.0, 1.0, 1.0)
         );
      double zz9 = reach;
      ...
      for (int zz10 = 0; zz10 < zz8.size(); zz10++) {
         ...
         if (zz11.canBeCollidedWith()) {
            float ex = (float)(zz11.getCollisionBorderSize() * HitBox.getExpand(zz11));
      ...
      return entity != null && hitVec != null ? new Object[]{entity, hitVec} : null;
   }
}
