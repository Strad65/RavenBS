...
public class HitBox extends Module {
   ...
   public HitBox() {
      ...
      this.registerSetting(this.showHitbox = new ButtonSetting("Show new hitbox", false));
   ...
   public void onRenderWorld(RenderWorldLastEvent e) {
      if (this.showHitbox.isToggled() && Utils.nullCheck()) {
         for (Entity en : mc.theWorld.loadedEntityList) {
            if (en != mc.thePlayer
               && en instanceof EntityLivingBase
   ...
   public Entity getEntity(float partialTicks) {
      if (mc.getRenderViewEntity() != null && mc.theWorld != null) {
         ...
         double d0 = mc.playerController.extendedReach()
            ? 6.0
            : (ModuleManager.reach.isEnabled() ? Utils.getRandomValue(Reach.min, Reach.max, Utils.getRandom()) : 3.0);
         this.mv = mc.getRenderViewEntity().rayTrace(d0, partialTicks);
         double d2 = d0;
         Vec3 vec3 = mc.getRenderViewEntity().getPositionEyes(partialTicks);
         if (this.mv != null) {
            d2 = this.mv.hitVec.distanceTo(vec3);
         }
         ...
         for (Object o : list) {
            ...
            if (entity.canBeCollidedWith()) {
               float ex = (float)(entity.getCollisionBorderSize() * getExpand(entity));
         ...
         if (this.pointedEntity != null && (d3 < d2 || this.mv == null)) {
            this.mv = new MovingObjectPosition(this.pointedEntity, vec6);
   ...
   private void rh(Entity e, Color c) {
      if (e instanceof EntityLivingBase) {
         ...
         float ex = (float)(e.getCollisionBorderSize() * multiplier.getInput());
