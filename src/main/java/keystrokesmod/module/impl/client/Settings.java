...
public class Settings extends Module {
   ...
   public Settings() {
      ...
      this.registerSetting(customCapes = new SliderSetting("Custom cape", 0, this.capes));
   ...
   public void loadCapes() {
      try {
         for (int i = 1; i < this.capes.length; i++) {
            String name = this.capes[i].toLowerCase();
            if (i > 1) {
               name = "rvn_" + name;
            }

            InputStream stream = keystrokesmod.Raven.class.getResourceAsStream("/assets/keystrokesmod/textures/capes/" + name + ".png");
            if (stream != null) {
