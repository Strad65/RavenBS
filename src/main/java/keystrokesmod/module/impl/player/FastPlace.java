...
public class FastPlace extends Module {
   ...
   public FastPlace() {
      ...
      this.registerSetting(this.tickDelay = new SliderSetting("Tick delay", 1.0, 1.0, 3.0, 1.0));
      ...
      this.registerSetting(this.disablePearl = new ButtonSetting("Disable while holding pearl", true));
   ...
   public void onPreUpdate(PreUpdateEvent e) {
      if (!ModuleManager.scaffold.stopFastPlace()) {
         if (Utils.nullCheck() && mc.inGameHasFocus) {
            ...
            if (this.pitchCheck.isToggled() && mc.thePlayer.rotationPitch < 70.0F) {
               return;
            }

            if (this.disablePearl.isToggled()
               && mc.thePlayer.getHeldItem() != null
               && mc.thePlayer.getHeldItem().getItem() instanceof ItemEnderPearl) {
               return;
            }

            int delay = (int)this.tickDelay.getInput();
            if (delay == 0) {
